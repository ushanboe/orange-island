...
export class ImmigrationSystem {
    constructor(game) {
        ...
        // === NEW LANDING SYSTEM ===
        // Registry of all valid sand tiles on main island perimeter
        this.mainIslandSandTiles = [];
        // Shuffled queue of available target tiles (not yet used)
        this.availableTargets = [];
        // Set of used target coordinates (as "x,y" strings)
        this.usedTargets = new Set();
        // Flag to track if registry has been built
        this.registryBuilt = false;  // Increased to prevent people not appearing after boat landing
    ...
    /**
     * Build registry of all valid sand tiles on main island perimeter
     * Called once after map is ready
     * Excludes source island tiles
     */
    buildSandTileRegistry() {
        const map = this.game.tileMap;
        if (!map || this.registryBuilt) return;
        
        console.log('[IMMIGRATION] Building sand tile registry...');
        
        const centerX = map.width / 2;
        const centerY = map.height / 2;
        
        // Get source island boundaries to exclude them
        const sourceIslandBounds = [];
        if (map.sourceIslands) {
            for (const island of map.sourceIslands) {
                sourceIslandBounds.push({
                    centerX: island.centerX,
                    centerY: island.centerY,
                    radius: 20  // Exclude tiles within 20 tiles of source island centers
                });
            }
        }
        
        // Scan entire map for sand tiles (terrain = 2)
        for (let y = 0; y < map.height; y++) {
            for (let x = 0; x < map.width; x++) {
                const terrain = map.getTerrainAt(x, y);
                
                // Only SAND tiles (terrain = 2) that are coastal
                if (terrain === 2 && map.isCoastal(x, y)) {
                    
                    // Check distance from main island center
                    const distFromCenter = Math.sqrt(
                        Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2)
                    );
                    
                    // Must be within reasonable distance of main island (not too far, not too close to center)
                    if (distFromCenter > 10 && distFromCenter < 55) {
                        
                        // Check if this tile is near any source island - EXCLUDE if so
                        let isNearSourceIsland = false;
                        for (const bounds of sourceIslandBounds) {
                            const distFromSource = Math.sqrt(
                                Math.pow(x - bounds.centerX, 2) + Math.pow(y - bounds.centerY, 2)
                            );
                            if (distFromSource < bounds.radius) {
                                isNearSourceIsland = true;
                                break;
                            }
                        }
                        
                        if (!isNearSourceIsland) {
                            // Find adjacent WATER tile - this makes it an OUTERMOST sand tile
                            const adjacentWater = this.findAdjacentWaterTile(x, y);

                            if (adjacentWater) {
                                // Valid outermost sand tile with adjacent water!
                                this.mainIslandSandTiles.push({ 
                                    sandX: x, 
                                    sandY: y,
                                    waterX: adjacentWater.x,
                                    waterY: adjacentWater.y
                                });
                            }
                        }
                    }
    ...
    /**
     * Find a water tile adjacent to the given sand tile
     * Prefers water tiles that are FURTHER from island center (outermost)
     */
    findAdjacentWaterTile(sandX, sandY) {
        const map = this.game.tileMap;
        if (!map) return null;
        ...
        let bestWater = null;
        let bestDistFromCenter = -1;

        for (const dir of directions) {
            const wx = sandX + dir.dx;
            const wy = sandY + dir.dy;

            // Check bounds
            if (wx < 0 || wy < 0 || wx >= map.width || wy >= map.height) continue;

            const terrain = map.getTerrainAt(wx, wy);

            // Water tiles are terrain 0 (deep) or 1 (shallow)
            if (terrain === 0 || terrain === 1) {
                // Calculate distance from center - prefer tiles FURTHER from center (outermost)
                const distFromCenter = Math.sqrt(
                    Math.pow(wx - centerX, 2) + Math.pow(wy - centerY, 2)
                );

                if (distFromCenter > bestDistFromCenter) {
     ...
     */
    resetTargetQueue() {
        // Copy all sand tiles to available targets
        this.availableTargets = [...this.mainIslandSandTiles];
        
        // Fisher-Yates shuffle
        for (let i = this.availableTargets.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [this.availableTargets[i], this.availableTargets[j]] = 
                [this.availableTargets[j], this.availableTargets[i]];
        }
        
        // Clear used targets
        this.usedTargets.clear();
        
        console.log(`[IMMIGRATION] Target queue reset with ${this.availableTargets.length} shuffled targets`);
    }
    ...
    trySpawnPeopleBoatFromIsland(sourceIsland) {
        ...
        // NEW: Boat navigates to WATER tile (which it can reach), lands there, spawns crowd on SAND tile
        // targetTile has: sandX, sandY (beach for crowd), waterX, waterY (for boat navigation)
        const landingSpot = { 
            sandX: targetTile.sandX,
            sandY: targetTile.sandY,
            waterX: targetTile.waterX,
            waterY: targetTile.waterY,
            // For backwards compatibility, x/y point to WATER tile (where boat navigates)
            x: targetTile.waterX + 0.5, 
            y: targetTile.waterY + 0.5 
        };
    ...
    spawnCrowdFromBoat(boat) {
        ...
        // === NEW SIMPLE SPAWN LOGIC ===
        // Boat's target is already a sand tile, spawn crowd there!
        const map = this.game.tileMap;
        let landX, landY;
        
        if (boat.targetLanding && boat.targetLanding.sandX !== undefined) {
            // Use the SAND tile coordinates for crowd spawning (not water tile!)
            landX = boat.targetLanding.sandX + 0.5;
            landY = boat.targetLanding.sandY + 0.5;
            console.log(`[IMMIGRATION] Using sand tile (${boat.targetLanding.sandX}, ${boat.targetLanding.sandY}) for crowd spawn`);
        } else if (boat.targetLanding) {
            // Fallback to old format (x, y)
            landX = boat.targetLanding.x;
            landY = boat.targetLanding.y;
        } else {
            // Fallback to boat position if no target
            landX = boat.x;
            landY = boat.y;
        }
        
        const terrain = map ? map.getTerrainAt(Math.floor(landX), Math.floor(landY)) : -1;
        console.log(`[IMMIGRATION] Spawning crowd at (${landX.toFixed(1)}, ${landY.toFixed(1)}) with ${boat.peopleCount} people. Target tile terrain=${terrain}`);
    ...
    render(ctx, offsetX, offsetY, tileSize) {
        // === DEBUG: Render boat target tiles if enabled ===
        if (this.game.debugShowBoatTargets) {
            this.renderDebugTargets(ctx, offsetX, offsetY, tileSize);
        }
    ...
    /**
     * Debug: Render target tiles for active boats
     */
    renderDebugTargets(ctx, offsetX, offsetY, tileSize) {
        ctx.save();

        for (const boat of this.peopleBoats) {
            if (!boat.targetLanding) continue;

            const target = boat.targetLanding;

            // Draw WATER tile (where boat navigates to) - ORANGE
            if (target.waterX !== undefined && target.waterY !== undefined) {
                const waterScreenX = offsetX + target.waterX * tileSize;
                const waterScreenY = offsetY + target.waterY * tileSize;

                ctx.fillStyle = 'rgba(255, 165, 0, 0.5)';  // Orange, semi-transparent
                ctx.fillRect(waterScreenX, waterScreenY, tileSize, tileSize);

                ctx.strokeStyle = 'rgba(255, 100, 0, 1)';
                ctx.lineWidth = 2;
                ctx.strokeRect(waterScreenX, waterScreenY, tileSize, tileSize);

                // Label
                ctx.fillStyle = 'white';
                ctx.font = '10px Arial';
                ctx.fillText('W', waterScreenX + 2, waterScreenY + 12);
            }

            // Draw SAND tile (where crowd spawns) - YELLOW
            if (target.sandX !== undefined && target.sandY !== undefined) {
                const sandScreenX = offsetX + target.sandX * tileSize;
                const sandScreenY = offsetY + target.sandY * tileSize;

                ctx.fillStyle = 'rgba(255, 255, 0, 0.5)';  // Yellow, semi-transparent
                ctx.fillRect(sandScreenX, sandScreenY, tileSize, tileSize);

                ctx.strokeStyle = 'rgba(200, 200, 0, 1)';
                ctx.lineWidth = 2;
                ctx.strokeRect(sandScreenX, sandScreenY, tileSize, tileSize);

                // Label
                ctx.fillStyle = 'black';
                ctx.font = '10px Arial';
                ctx.fillText('S', sandScreenX + 2, sandScreenY + 12);
            }

            // Draw line from boat to target
            const boatScreenX = offsetX + boat.x * tileSize + tileSize / 2;
            const boatScreenY = offsetY + boat.y * tileSize + tileSize / 2;
            const targetScreenX = offsetX + (target.waterX || target.x) * tileSize + tileSize / 2;
            const targetScreenY = offsetY + (target.waterY || target.y) * tileSize + tileSize / 2;

            ctx.strokeStyle = 'rgba(255, 0, 255, 0.7)';  // Magenta line
            ctx.lineWidth = 1;
            ctx.setLineDash([5, 5]);
            ctx.beginPath();
...
export class PeopleBoat {
    ...
    moveTowardsTarget() {
        ...
        // === NEW SIMPLE LANDING LOGIC ===
        // Land when within 1.5 tiles of target sand tile OR on/adjacent to target
        if (dist < 1.5) {
            console.log(`[BOATS] People boat reached target at (${this.targetLanding.x.toFixed(1)}, ${this.targetLanding.y.toFixed(1)}), landing!`);
            this.state = 'landed';
            return;
        }
        
        // Also land if we're on the target tile or adjacent to it
        const targetTileX = Math.floor(this.targetLanding.x);
        const targetTileY = Math.floor(this.targetLanding.y);
        const boatTileX = Math.floor(this.x);
        const boatTileY = Math.floor(this.y);
        
        if (Math.abs(boatTileX - targetTileX) <= 1 && Math.abs(boatTileY - targetTileY) <= 1) {
            console.log(`[BOATS] People boat adjacent to target tile, landing!`);
            this.state = 'landed';
            return;
        }
        ...
        if (!this.lastPos) {
            ...
            if (movedDist < 0.1) {  // Lowered from 0.3 to 0.1 for slow boats
                ...
                if (this.stuckFrames > 150) {
                    ...
                    console.log(`[BOATS] People boat stuck, retry attempt ${this.retryAttempts}/6`);
                    
                    // After 6 retry attempts, return to source and get new target
                    if (this.retryAttempts >= 6) {
                        console.log('[BOATS] People boat giving up after 6 attempts, returning to source for new target');
                        ...
                        if (immigrationSystem) {
                            ...
                            if (newTarget) {
                                ...
                                this.targetLanding = { 
                                    sandX: newTarget.sandX,
                                    sandY: newTarget.sandY,
                                    waterX: newTarget.waterX,
                                    waterY: newTarget.waterY,
                                    x: newTarget.waterX + 0.5, 
                                    y: newTarget.waterY + 0.5 
                                };
                                console.log(`[BOATS] Got new target: sand(${newTarget.sandX}, ${newTarget.sandY}), water(${newTarget.waterX}, ${newTarget.waterY})`);
                            }
                        }
                        
                        // Return to source first, then try again
                        this.state = 'leaving';
                        this.retryAttempts = 0;
                        return;
                    }
        ...
        // Check if next position is water - if so, move there
        if (this.isWater(nextX, nextY)) {
            this.x = nextX;
            this.y = nextY;
        } else {
            // Hit land! Check if we're close enough to target to land
            if (dist < 5) {
                console.log(`[BOATS] People boat hit land near target, landing!`);
                this.state = 'landed';
            } else {
                // Not close to target, try to navigate around
                const clearDir = this.findClearDirection(-targetDirX, -targetDirY);
                if (clearDir.angle !== 0) {
